    ...
    <>
      ...
      <main className="py-16 px-6">
        <div className="max-w-3xl mx-auto">
          {/* Back Link */}
          <Link href="/blog"
            className="flex items-center gap-2 text-blue-600 hover:underline mb-8 text-sm font-medium">
            <FiArrowLeft /> Back to Blog
          </Link>
          ...
          {blog.tags?.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {blog.tags.map((tag, i) => (
                <span key={i}
                  className="text-xs bg-blue-50 text-blue-600 px-3 py-1 rounded-full font-medium flex items-center gap-1">
                  <FiTag size={10} /> {tag}
                </span>
              ))}
            </div>
