...
export default function ManagePortfolioPage() {
  ...
  const handleEdit = (item) => {
    setForm({ title: item.title, description: item.description, image: item.image || '',
      ...
      liveUrl: item.liveUrl || '', githubUrl: item.githubUrl || '', isActive: item.isActive });
    setEditId(item._id);
    setShowForm(true);
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input placeholder="https://myproject.com" value={form.liveUrl} onChange={set('liveUrl')}
                      className="input-field" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">GitHub URL</label>
                    <input placeholder="https://github.com/user/repo" value={form.githubUrl} onChange={set('githubUrl')}
                      className="input-field" />
                  </div>
                </div>
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex gap-3 text-xs text-gray-400">
                        {item.liveUrl && <a href={item.liveUrl} target="_blank" rel="noreferrer"
                          className="flex items-center gap-1 text-blue-500 hover:underline"><FiExternalLink size={12}/> Live</a>}
                        {item.githubUrl && <a href={item.githubUrl} target="_blank" rel="noreferrer"
