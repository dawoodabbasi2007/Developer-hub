              ...
              <div className="grid md:grid-cols-2 gap-8">
                  ...
                  {recentInquiries.length === 0 ? (
                    ...
                    <ul className="space-y-3">
                      {recentInquiries.map(inq => (
                        <li key={inq._id} className="flex items-center justify-between py-2 border-b last:border-0">
                          <div>
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                            ...
                            inq.status === 'read'    ? 'bg-yellow-100 text-yellow-600' :
                ...
                <div className="bg-white rounded-2xl shadow-sm p-6">
                  ...
                  {recentMeetings.length === 0 ? (
                    <p className="text-gray-400 text-sm">No meetings booked yet.</p>
                  ) : (
                    <ul className="space-y-3">
                      {recentMeetings.map(m => (
                        <li key={m._id} className="flex items-center justify-between py-2 border-b last:border-0">
                          <div>
                            <p className="font-medium text-sm text-gray-800">{m.name}</p>
                            <p className="text-xs text-gray-400">{m.date} at {m.time}</p>
                          </div>
                          <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                            m.status === 'pending'   ? 'bg-yellow-100 text-yellow-600' :
                            m.status === 'confirmed' ? 'bg-green-100 text-green-600'   :
